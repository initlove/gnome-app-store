...
static void generate_online_apps (AppShellData * app_data);	//dliang
static void insert_launcher_into_online_category (CategoryData * cat_data, GnomeDesktopItem * desktop_item,
	AppShellData * app_data);				//dliang
...
static void online_tile_activated_cb (Tile * tile, TileEvent * event, gpointer user_data);	//dliang
static void handle_launcher_single_clicked (Tile * launcher, gpointer data);
static void online_handle_launcher_single_clicked (Tile * launcher, gpointer data);	//dliang
...
show_no_results_message (AppShellData * app_data, GtkWidget * containing_vbox)
{
	...
	str1 = g_strdup_printf ("%s", app_data->filter_string);
...
void
load_app (GHashTable *apps_cache, xmlNodePtr app_node)
{
	...
	char *p;
        ...
        for (node = app_node->xmlChildrenNode; node; node = node->next) {
				...
				if (p)
					...
					p = content;
				g_hash_table_insert (apps_cache, g_strdup(p), g_strdup (content));
...
void
load_online_apps (GHashTable *apps_cache)
{
        xmlDocPtr doc_ptr;
        ...
        doc_ptr = xmlParseFile ("/tmp/appdata.xml");
...
void
add_to_online_apps_cate (const char *key, const char *value, AppShellData *app_data)
{
	/*FIXME: filter screensavers' */
	if (strstr (value, "screensaver"))
		return;
	GList *l;
	CategoryData *cate, *online_apps_category;
	online_apps_category = NULL;
	for (l = app_data->categories_list; l; l = l->next) {
		cate = l->data;
		if (strcmp (cate->category, "Online apps") == 0) {
			online_apps_category = cate;
			break;
		}
	}
	
	if (!online_apps_category)
		return;

	GnomeDesktopItem *item;
	gchar *desktop_file;
	
	desktop_file = g_build_filename (g_get_home_dir (), "/Desktop/gnome-appstore/database/applications", value, NULL);
	GError *error = NULL;
	item = gnome_desktop_item_new_from_file (desktop_file, 0, &error);
	if (error) {
		printf ("error is %s\n", error->message);
                g_error_free (error);
	} else if (gnome_desktop_item_get_entry_type (item) == GNOME_DESKTOP_ITEM_TYPE_APPLICATION) {
		gnome_desktop_item_set_string (item, GNOME_DESKTOP_ITEM_ICON, "accessories-text-editor");
		insert_launcher_into_online_category (online_apps_category, item, app_data);
	}
}

static void
generate_online_apps (AppShellData * app_data)
{
	GHashTable *all_apps_cache = NULL;
	GError *error = NULL;

	CategoryData *online_apps_category = NULL;
	GList *categories, *launchers;

	all_apps_cache = g_hash_table_new (g_str_hash, g_str_equal);

	load_online_apps (all_apps_cache);
	...
	for (categories = app_data->categories_list; categories; categories = categories->next)
	{
		...
		for (launchers = cat_data->launcher_list; launchers; launchers = launchers->next)
		{
			...
			if (p)
				...
				p = uri;
			if (g_hash_table_lookup (all_apps_cache, p))
			{
				g_hash_table_remove (all_apps_cache, p);
			}
		}
	}
/*FIXME: if we got some apps in all_apps_cache */

						
	online_apps_category = g_new0 (CategoryData, 1);
	online_apps_category->category = g_strdup ("Online apps");
				
	app_data->categories_list =
			g_list_prepend (app_data->categories_list, online_apps_category);

        g_hash_table_foreach (all_apps_cache,  add_to_online_apps_cate,
				app_data);

	
//	insert_launcher_into_category (new_apps_category, item, app_data);
...
static void
insert_launcher_into_online_category (CategoryData * cat_data, GnomeDesktopItem * desktop_item,
	AppShellData * app_data)
{
	GtkWidget *launcher;
	...
	g_signal_connect (launcher, "tile-activated", G_CALLBACK (online_tile_activated_cb), app_data);
...
static void
online_tile_activated_cb (Tile * tile, TileEvent * event, gpointer user_data)
{
	switch (event->type)
	{
	case TILE_EVENT_ACTIVATED_SINGLE_CLICK:
	case TILE_EVENT_ACTIVATED_KEYBOARD:
		online_handle_launcher_single_clicked (tile, user_data);
		break;
	default:
		break;
	}
...
void
init_dr_table (GHashTable *dr_table)
{
        FILE *fp;
        char buf [1024];
        char *p;
        ...
        fp = fopen ("/home/novell/Desktop/gnome-appstore/rpm-desktop", "r");
        if (fp == NULL) {
        ...
        while (!feof (fp)) {
                ...
                desktop_name = strrchr (p + strlen (":/usr/share/"), '/');
		desktop_name ++;
                p = strchr (desktop_name, '\n');
                if (p)
                        *p = 0;
		gboolean dash = FALSE;
		for (p = rpm_name+1; *p; p++) {
			if (*p == '-')
				dash = TRUE;
			else {
				if (dash == TRUE) {
                ...
                g_hash_table_insert (dr_table, g_strdup (desktop_name), g_strdup (rpm_name + 1));
        }
	fclose (fp);
}

const gchar *
get_package_name (gchar *uri)
{
	gchar *basename;
	...
	static GHashTable *desktop_rpm_table = NULL;
	if (!desktop_rpm_table) {
		desktop_rpm_table = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, NULL);
		init_dr_table (desktop_rpm_table);
	}

	return g_hash_table_lookup (desktop_rpm_table, basename);
}

static void
online_handle_launcher_single_clicked (Tile * launcher, gpointer data)
{
	AppShellData *app_data = (AppShellData *) data;
	gchar *gconf_key;
	const gchar *pkgname;
	pkgname = get_package_name (launcher->uri);
	if (pkgname) {
		gchar *cmd;
