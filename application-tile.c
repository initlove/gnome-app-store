...
//dliang
GtkImage *
load_from_online_icon_cache (gchar *image_id)
{
	static GHashTable *online_icon_cache = NULL;

	if (online_icon_cache == NULL) {
		online_icon_cache = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, g_free);
	        GDir *dir;
		gchar *dirname;

		dirname = "/home/novell/Desktop/gnome-appstore/icons";
		...
		if (!dir) {
			...
			if (p)
				*p = 0;
			g_hash_table_insert (online_icon_cache, content, filename);
	        }
        	g_dir_close (dir);
	} else {
		gchar *uri;
		uri = g_hash_table_lookup (online_icon_cache, image_id);
		if (uri)
			return GTK_IMAGE (gtk_image_new_from_file (uri));	
		else
			return NULL;
	}
}
...
application_tile_setup (ApplicationTile *this, const gchar *gconf_prefix)
{
	...
	GtkImage *img_hack;
//	if (!load_image_by_id (img_hack, priv->image_size, priv->image_id)) {
//	}
	if (priv->image_id) {
		image = load_from_online_icon_cache (priv->image_id);
	}
	if (!image)
	...
	g_object_set (
		...
		NULL);
	gtk_widget_set_tooltip_text (GTK_WIDGET (this), comment == NULL && !priv->show_generic_name ? desc : comment);
...
get_desktop_item_startup_status (GnomeDesktopItem *desktop_item)
{
	...
	if (gnome_desktop_item_get_string (desktop_item, "X-GNOME-Provides")) {
		g_free (filename);
		return APP_NOT_ELIGIBLE;
	}
