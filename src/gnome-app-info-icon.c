		...
		gpointer      userdata)
{
	...
	switch (event->type)
	{
		case CLUTTER_BUTTON_PRESS:
			...
			gnome_app_stage_load (app_stage, GNOME_APP_STAGE_LOAD_NEW, "GnomeAppInfoPage", "data", priv->info, NULL);
			break;
		case CLUTTER_ENTER:
...
printf ("curl %s > '%s'\n", open_result_get (info, "icon"), open_result_get (info, "name"));
	gnome_app_set_icon (pic, open_result_get (info, "icon"));
