...
return ;
	...
	//TODO
	group_height = 300;
printf ("y %f %f height %f  group height %f\n", y, clutter_actor_get_y (text), height, group_height);
	if (y > 0) {
...
gnome_app_info_page_new_with_app (OpenResult *info)
{
	...
	/* TODO: will we have more than three pics ? */
	if (count > 1) {
		// add small icon or make big pic a new widget 
	}
	gnome_app_ui_set_icon (big_pic, open_result_get (info, "previewpic1"));
	...
	g_signal_connect (download_button, "button-press-event", G_CALLBACK (on_download_button_press), NULL);
	g_signal_connect (return_button, "button-press-event", G_CALLBACK (on_return_button_press), NULL);
